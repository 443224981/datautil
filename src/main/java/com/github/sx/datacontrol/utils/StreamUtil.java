...
public class StreamUtil {
     ...
     * @return
     */
    public static <T, D extends Collection<T>> List<T> pageList(D data, Integer pageNum, Integer pageSize) {
        if (DataUtil.isNotEmpty(data)) {
            pageNum = DataUtil.isNull(pageNum) ? 1 : pageNum;
            pageSize = DataUtil.isNull(pageSize) ? 10 : pageSize;
            AssertUtil.isTrue(pageNum > 0, "pageNum must bigger than 0!");
            AssertUtil.isTrue(pageSize > 0, "pageSize must bigger than 0!");
            return data.stream().skip((pageNum - 1) * pageSize).limit(pageSize).collect(Collectors.toList());
        }
